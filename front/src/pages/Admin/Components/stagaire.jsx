...
const StagiairesAcceptes = () => {
    ...
    const handleSupervisorChange = async (internId, supervisorId, sujetId) => {
        try {
            await axiosInstance.put(`candidature/encadrant/${supervisorId}/candidat/${internId}/sujet/${sujetId}`);
            fetchInterns();
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {interns && interns.map((intern) => (
                            <tr key={intern.id}>
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{intern.user?.nom || 'N/A'}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{intern.user?.email || 'N/A'}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{intern.user?.telephone || 'N/A'}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{intern.sujet?.titre || 'N/A'}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <button onClick={() => handleDownloadCV(intern.user?.id)} className="text-blue-600 hover:text-blue-800">Télécharger CV</button>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm">
                                    <select
                                        value={intern.supervisorId ? intern.supervisorId : ''}
                                        onChange={(e) => handleSupervisorChange(intern.user?.id, e.target.value, intern.sujetId)}
                                        className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"
                                    >
                                        <option value="">Assigner un superviseur</option>
                                        {supervisors.map((supervisor) => (
                                            <option key={supervisor.id} value={supervisor.id}>{supervisor.nom} {supervisor.prenom}</option>
                                        ))}
                                    </select>
                                </td>
                            </tr>
